...
if (!@is_dir($root_path)) {
    echo sprintf('<h1>找不到根路径 "%s" !</h1>', fm_enc($root_path));
    exit;
}
...
if (isset($_GET['new'])) {
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if (fm_mkdir($path . '/' . $new, false) === true) {
            fm_set_msg(sprintf('文件夹 <b>%s</b> 创建成功', fm_enc($new)));
        } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
            fm_set_msg(sprintf('文件夹 <b>%s</b> 已存在', fm_enc($new)), 'alert');
        } else {
            fm_set_msg(sprintf('文件夹 <b>%s</b> 创建失败', fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('文件夹名称错误', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                ...
                fm_set_msg('具有此路径的文件或者文件夹已存在', 'alert');
            } else {
                fm_set_msg(sprintf('从 <b>%s</b> 移动到 <b>%s</b> 时出错', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else {
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('从 <b>%s</b> 复制到 <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf('从 <b>%s</b> 复制到 <b>%s</b> 时出错', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
        fm_set_msg('路径不能相同', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? '所选文件和文件夹已移动' : 'Selected files and folders copied';
            fm_set_msg($msg);
        } else {
            $msg = $move ? '移动项目时出错' : 'Error while copying items';
...
if (isset($_GET['ren'], $_GET['to'])) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('从 <b>%s</b> 重命名为 <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('将 <b>%s</b> 重命名为 <b>%s</b> 时出错', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
...
if (isset($_POST['upl'])) {
    ...
    if ($errors == 0 && $uploads > 0) {
        fm_set_msg(sprintf('所有上传到 <b>%s</b> 的文件', fm_enc($path)));
    } elseif ($errors == 0 && $uploads == 0) {
        fm_set_msg('未上传任何文件', 'alert');
    } else {
        fm_set_msg(sprintf('上传文件时出错: %s', $uploads), 'error');
    }
...
if (isset($_POST['group'], $_POST['delete'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('已删除所选文件和文件夹');
        } else {
            fm_set_msg('删除项目时出错', 'error');
        }
    } else {
        fm_set_msg('未选择任何内容', 'alert');
    }
...
if (isset($_POST['group'], $_POST['zip'])) {
    ...
    if (!empty($files)) {
        ...
        if ($res) {
            fm_set_msg(sprintf('已创建存档 <b>%s</b>', fm_enc($zipname)));
        } else {
            fm_set_msg('未创建存档', 'error');
        }
    } else {
        fm_set_msg('未选择任何内容', 'alert');
    }
...
if (isset($_POST['copy'])) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            ?>
            <p class="break-word">文件夹: <b><?php echo implode('</b>, <b>', $copy_files_enc) ?></b></p>
            <p class="break-word">源文件夹: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
            ...
            <p>
                <button class="btn"><i class="icon-apply"></i> 复制</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> 取消</a></b>
            </p>
        </form>
    </div>
    ...
    <div class="path">
        ...
        <p class="break-word">
            ...
            if ($is_zip && $filenames !== false) {
                ...
                ?>
                存档中的文件: <?php echo $total_files ?><br>
                总尺寸: <?php echo fm_get_filesize($total_uncomp) ?><br>
                存档中的大小: <?php echo fm_get_filesize($total_comp) ?><br>
...
if (isset($_GET['chmod']) && !FM_IS_WIN) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <p>
                <button class="btn"><i class="icon-apply"></i> 变更</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> 取消</a></b>
            </p>
...
<table><tr>
<th style="width:3%"><label><input type="checkbox" title="反向选择" onclick="checkbox_toggle()"></label></th>
...
<?php if (!FM_IS_WIN): ?>
<td><a title="修改权限" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></td>
<td><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
<?php endif; ?>
<td>
<a title="删除" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('删除文件夹？');"><i class="icon-cross"></i></a>
<a title="重命名" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
<a title="复制到..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i></a>
...
<tr>
<td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
<td><div class="filename"><a href="<?php echo fm_enc($filelink) ?>" title="文件简介"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div></td>
<td><span class="gray" title="<?php printf('%s bytes', $filesize_raw) ?>"><?php echo $filesize ?></span></td>
<td><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
<td><a title="权限修改" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></td>
<td><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
<?php endif; ?>
<td>
<a title="删除" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('删除文件?');"><i class="icon-cross"></i></a>
<a title="重命名" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
...
实际大小: <span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></span>,
...
<a href="#" onclick="unselect_all();return false;"><i class="icon-checkbox_uncheck"></i> 取消全选</a> &nbsp;
<a href="#" onclick="invert_all();return false;"><i class="icon-checkbox_invert"></i> 反向选择</a></p>
<p><input type="submit" name="delete" value="删除" onclick="return confirm('删除所选文件和文件夹?')">
<input type="submit" name="zip" value="压缩" onclick="return confirm('创建压缩包?')">
...
<a title="新增文件夹" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"><i class="icon-folder_add"></i></a>
<?php if (FM_USE_AUTH): ?><a title="注销" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
</div>
        <?php
        $path = fm_clean_path($path);
